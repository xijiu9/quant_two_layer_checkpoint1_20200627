...
def train_loop(model_and_loss, optimizer, lr_scheduler, train_loader, val_loader, debug_loader, epochs, fp16, logger,
    ...
    for epoch in epoch_iter:
        ...
        print("time cost:{} s".format(start - end))
        if not skip_validation:
            ...
            plt.figure(1)
            plt.title("validation_{:.3f}".format(best_prec1))
            plt.ylim(80, 95)
            plt.scatter(epoch_history, valid_history, s=1)
            plt.savefig(args.workspace + "/validation.png")
        if save_checkpoints and (not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0):
            if not skip_training:
                if not os.path.exists(checkpoint_dir + '/models'):
                    os.makedirs(checkpoint_dir + '/models')
                ...
                }, is_best, checkpoint_dir=checkpoint_dir + '/models', backup_filename=backup_filename)
