...
def add_parser_arguments(parser):
    ...
    parser.add_argument('--bwbits_l', type=int, default=4, help='backward weight number of bits for linear')
    parser.add_argument('--bwbits_c', type=int, default=4, help='backward weight number of bits for conv')
...
def main(args):
    ...
    config.bweight_num_bits_c = args.bwbits_c
    config.bweight_num_bits_l = args.bwbits_l
    ...
    if args.optimizer_batch_size < 0:
        ...
        if args.optimizer_batch_size % tbs != 0:
            print("Warning: simulated batch size {} is not divisible by actual batch size {}".format(
    ...
    if args.pretrained_weights:
        if os.path.isfile(args.pretrained_weights):
            ...
            print("=> no pretrained weights found at '{}', {}".format(args.resume, args.pretrained_weights))
    ...
    print("loading data...")
    train_loader, train_loader_len = get_train_loader(args.data, args.batch_size, 1000, args.mixup > 0.0,
                                                      workers=args.workers, fp16=args.fp16)
    if args.mixup != 0.0:
        train_loader = MixUpWrapper(args.mixup, 1000, train_loader)

    val_loader, val_loader_len = get_val_loader(args.data, args.batch_size, 1000, False, workers=args.workers,
                                                fp16=args.fp16)
    debug_loader, debug_loader_len = get_debug_loader(args.data, args.batch_size, 1000, False, workers=args.workers,
                                                      fp16=args.fp16)
